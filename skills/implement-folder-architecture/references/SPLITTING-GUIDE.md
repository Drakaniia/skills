...
When the `implement-folder-architecture` skill processes an audit report that flags a file as oversized (>400 lines), the implementing AI session must split it into smaller modules. This guide provides **language-specific mechanics** for doing so correctly.
